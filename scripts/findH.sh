...
do
    ...
    do
        ...
        $SCRIPT_DIR/../bin/WOAH ionosphere.arff b g 1 $1 $2 $j $i 1
        echo "[START-$j w $i]: Doing WOAH search in parkinson.arff"
        $SCRIPT_DIR/../bin/WOAH parkinsons.arff 1 2 1 $1 $2 $j $i 1
        echo "[START-$j w $i]: Doing WOAH search in spectf-heart.arff"
        $SCRIPT_DIR/../bin/WOAH spectf-heart.arff 1 2 1 $1 $2 $j $i 1
    done;
